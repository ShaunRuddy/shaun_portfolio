(function () {
    ...
    navigation.forEach((item) => {
      ...
      if (!href.startsWith("#")) {
        if (href.startsWith("mailto:") || href.startsWith("tel:")) {
          /* default */
        } else if (/\.pdf$/i.test(href) || item.download === true) {
          a.setAttribute("download", "");
          a.setAttribute(
            "aria-label",
            `${item.label} (PDF download)`,
          );
        } else {
          a.target = "_blank";
  ...
  function isGithubCta(btn) {
    if (btn && btn.icon === "github") return true;
    const href = String(btn.href || "").toLowerCase();
  ...
  function renderHero(data) {
    ...
    h.cta.forEach((btn) => {
      ...
      const extAttrs = external
        ? ' target="_blank" rel="noopener noreferrer"'
        : "";
      if (isGithubCta(btn)) {
        const githubLabel =
          btn.label != null && String(btn.label).trim()
            ? String(btn.label).trim()
            : "";
        const githubAria =
          githubLabel ||
          (btn.ariaLabel != null && String(btn.ariaLabel).trim()
            ? String(btn.ariaLabel).trim()
            : "GitHub");
        const iconOnlyClass = githubLabel ? "" : " btn-cta-github-icon-only";
        const labelSpan = githubLabel
          ? `<span class="btn-cta-github-label">${escapeHtml(githubLabel)}</span>`
          : "";
        ctaHtml += `<a href="${escapeHtml(btn.href)}" class="${cls} btn-cta-github${iconOnlyClass}"${extAttrs} aria-label="${escapeHtml(githubAria)}">${GITHUB_ICON_SVG}${labelSpan}</a>`;
      } else {
        ctaHtml += `<a href="${escapeHtml(btn.href)}" class="${cls}"${extAttrs}>${escapeHtml(btn.label)}</a>`;
      }
    });
